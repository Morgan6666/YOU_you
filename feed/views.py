...
class UserPostListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        user = get_object_or_404(User, username = self.kwargs.get('username'))
        return Post.objects.filter(user_name = user).order_by('-date_pasted')
...
def search_posts(request):
    ...
    liked = [ i for i in object_list if Like.objets.filter(user = request.user, post = i)]
    context = {
        'posts': object_list,
        'liked_post': liked
    }
